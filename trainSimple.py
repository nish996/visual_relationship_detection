...
from keras.layers import Input, Dense, Lambda, merge, Dropout, BatchNormalization
...
temp_h_q = Dense(interNo, activation='tanh')(input_ic)
h_q = BatchNormalization()(temp_h_q)
#h_q_zd = Dropout(rate=0.7)(temp_h_q)
#h_q = Dense(interNo, activation='relu')(h_q_zd)
mu = Dense(n_z, activation='tanh')(h_q)
log_sigma = Dense(n_z, activation='tanh')(h_q)
...
decoder_hidden = Dense(1024, activation='tanh')
...
trainClasses = [allLabels[x.split()[0]] for x in open(path + 'trainclasses.txt').readlines()]
...
# noise_gen = np.zeros([totalExs , n_z])
noise_gen = np.random.normal(size=(totalExs , n_z))
...
np.save(open('pseudoTrainData' , 'w') , pseudoTrainData )
np.save(open('pseudoTrainLabels' , 'w')  , pseudoTrainLabels)
